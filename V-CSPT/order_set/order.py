...
class CSPTLIB:
    ...
    def output_data(self):
        ...
        """
        # 输出评价数据
        RD = RateData(self.inbound_trajectorydata, self.inter_location_inbound, 0)
        self.inbound_rate_data = RD.output_rate_data()
        RD = RateData(self.outbound_trajectorydata, self.inter_location_outbound, 0, self.arterial_length)
    ...
    def initial_performance_grade(self, inter_traffic_volume, ari_traffic_volume, lane_side):
        ...
        per = RateCoordinationPerformance(ratedata, inter_location, inter_traffic_volume, ari_traffic_volume,
                                          (self.inter_lane_num_inbound, self.inter_lane_num_outbound), lane_side)
        ...
        self.in_stop_percent = output_stop_percent(self.inter_location_inbound, self.inbound_trajectorydata, 0)
        self.out_stop_percent = output_stop_percent(self.inter_location_outbound, self.outbound_trajectorydata, 0)
...
class ExternalDataAnalyses:
    def __init__(self, loadData: list, show_interval: list):
        """
        :param loadData: 导入数据
        ...
        self.stop_threshold = 5  # 定义停车阈值为5km/h
        self.inbound_trajectorydata = None
        self.outbound_trajectorydata = None
    ...
    def output_data(self):
        ...
        """
        # 输出评价数据
        RD = RateData(self.inbound_trajectorydata, self.inter_location_inbound, self.stop_threshold)
        self.inbound_rate_data = RD.output_rate_data()
        RD = RateData(self.outbound_trajectorydata, self.inter_location_outbound, self.stop_threshold,
