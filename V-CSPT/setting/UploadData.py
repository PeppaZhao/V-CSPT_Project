...
class UpLoadData(QDialog):
    ...
    def __init__(self):
        ...
        self.inbound_trajectory = None
        self.outbound_trajectory = None
        self.setupUi(self)
        # 储存数据以供修改
    ...
    def setupUi(self, Dialog):
        ...
        if os.path.exists('./.inif/loaddataconfig.ini'):
            ...
            # if self.data_loc_file != ['', '']:
            self.color_font = ['#36ab60', '#36ab60']  # 初始化字体颜色
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_2.setText(_translate("Dialog", self.data_loc_file[1]))
        self.label.setText(_translate("Dialog", self.data_loc_file[0]))
    ...
    def open_file_in(self):
        self.inbound_trajectory = order.selectFile("CSV文件 PKL文件 (*.csv *.pkl)")  # 文件位置
        # 数据地址
    ...
    def open_file_out(self):
        self.outbound_trajectory = order.selectFile("CSV文件 PKL文件 (*.csv *.pkl)")  # 文件位置
    ...
    def reSetting(self, button):
        ...
        if button == self.buttonBox.button(QtWidgets.QDialogButtonBox.StandardButton.Ok):
            ...
            """
            # 获取输入框数据
            loadData = [self.cycle_input.text(), self.offset_input.text(), self.inbound_phase_input.text(),
                        self.outbound_phase_input.text(), self.signal_RG1_input.text(),
                        self.signal_RG2_input.text(), self.green_speed_input.text(),
                        self.collect_data_period_input.text(), self.arterial_length_input.text(),
                        self.side_lanenum_input.text(),
                        self.arterial_lanenum_input.text(), self.veh_volumes_input.text(),
                        self.inboundinter_loc_input.text(), self.outboundinter_loc_input.text(),
                        ]
            ...
            if res:
                ...
                are_all_lists = all(
                    isinstance(eval(loadData[i], {"__builtins__": {}}, {}), list) for i in [0, 1, 2, 3, 9, 12, 13])
                are_all_dir = all(
                    isinstance(eval(loadData[i], {"__builtins__": {}}, {}), dict) for i in [4, 5])
                are_all_int = all(
                    isinstance(eval(loadData[i], {"__builtins__": {}}, {}), int) for i in [6, 7])
                are_all_tuple = all(
                    isinstance(eval(loadData[i], {"__builtins__": {}}, {}), tuple) for i in [10, 11])
                are_all_float = isinstance(eval(loadData[8], {"__builtins__": {}}, {}), float) or isinstance(
                    eval(loadData[8], {"__builtins__": {}}, {}), int)
                # 如果非空则关闭窗口并更新配置数据
                if are_all_lists and are_all_dir and are_all_int and are_all_tuple and are_all_float:
                    time = QDateTime.currentDateTime()  # 获取当前时间，并存储在self.qpp_data
                    self.app_data.setValue('time', time.toString())  # 数据0：time.toString()为字符串类型
                    # 读取默认数据并存储
                    loadDataStr = ['cycle_input', 'offset_input', 'inbound_phase_input',
