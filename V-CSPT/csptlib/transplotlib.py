...
class SignalPlanPlot(AterialDataCollection):
    ...
    @staticmethod
    def set_label(ylabel: str = '距离 (m)', xlabel: str = '时间 (s)', fontsize: int = 20) -> None:
        """
        x轴、y轴标签及字体大小
...
class PurduePlot(SignalPlanPlot):
    ...
    @staticmethod
    def set_label(ylabel: str = '周期内到达时间 (s)', xlabel: str = '时间 (s)', fontsize: int = 12) -> None:
        """
        x轴、y轴标签及字体大小
    ...
    @staticmethod
    def _percent_on_green(value_sum: list, bound_BOG_EOG: tuple):
        pog = []
        ...
        return sum(pog) / len(pog)  # 返回 percent on green

    def _process_timedata(self, veh_ID_data, k):
        arrivel_time = [i + k * self.period_ for i in veh_ID_data[0]]
    ...
    def _plot_purdue(self, purdue_data: dict, size_dot: float, color_dot: str, label: str, title: str,
        ...
        for key_sum, value_sum in purdue_data.items():  # 第key下有i个list用于存储不同种子下的车辆到达数据
            ...
            transplt.xlim(0, self.period_ * self.seed_num)
            ...
            if POG:
                ...
                transplt.title(f'{key_sum} Purdue Coordination Diagram', fontsize=fontsize) if title == "E" \
                    else transplt.title(f'{key_sum} 普渡图({label_} POG = {round(percent_on_green * 100, 2)}%)',
                                        fontsize=fontsize)
            transplt.tight_layout()
            # transplt.show()
            transplt.savefig(f'csptlib\\res\\{key_sum} purdue_{label}.png', format='png')
            transplt.close('all')
    ...
    def plot_purdue(self, /, **kwargs):
        ...
        # 储存数据
        self.save_purdue_data('csptlib\\res')
        self.save_purdue_data('csptlib\\res')

        return self.purdue_data_inbound, self.purdue_data_outbound, self.inbound_BOG_EOG, self.outbound_BOG_EOG
...
if __name__ == '__main__':
    ...
    # tra.ari_signal_plan_ring1 = {'yr1': [3.0, 2.0, 2.0, 2.0], 'green1': [29.0, 43.0, 43.0, 52.0],
    #                              'yr2': [2.0, 2.0, 2.0, 2.0], 'green2': [39.0, 37.0, 38.0, 50.0],
    #                              'red': [75.0, 64.0, 63.0, 42.0]}
    # tra.ari_signal_plan_ring2 = {'yr1': [3.0, 2.0, 2.0, 2.0], 'green1': [43.0, 39.0, 25.0, 39.0],
    ...
    # purdue.ari_signal_plan_ring1 = {'yr1': [3.0, 2.0, 2.0, 2.0], 'green1': [29.0, 43.0, 43.0, 52.0],
    #                                 'yr2': [2.0, 2.0, 2.0, 2.0], 'green2': [39.0, 37.0, 38.0, 50.0],
    #                                 'red': [75.0, 64.0, 63.0, 42.0]}
    # purdue.ari_signal_plan_ring2 = {'yr1': [3.0, 2.0, 2.0, 2.0], 'green1': [43.0, 39.0, 25.0, 39.0],
    #                                 'yr2': [2.0, 2.0, 2.0, 2.0], 'green2': [25.0, 41.0, 56.0, 63.0],
    #                                 'red': [75.0, 64.0, 63.0, 42.0]}
    # # 绘图
    # purdue.plot_purdue()
