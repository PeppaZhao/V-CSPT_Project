...
from csptlib.datastore import AterialDataCollection, save_variable
from typing import Any, Generator
import numpy as np


class GetVisTrajectoryData(AterialDataCollection):
    def __init__(self, LoadNet_name_: str, LoadLayout_name_: str, link_num_inbound_: list, link_num_outbound_: list, /,
    ...
    def get_vis_trajectorydata(self) -> tuple[dict[str, tuple[Any, Any, Any]], dict[str, tuple[Any, Any, Any]]]:
        ...
        if len(self.seed) > 2:
            ...
            for s in range(self.seed[0], self.seed[1] + 1, self.step):
                ...
                yield s
        # 存储轨迹数据
        save_variable(inbound_traydata, 'inbound_trajectorydata', 'csptlib\\res')
        save_variable(outbound_tradata, 'outbound_trajectorydata', 'csptlib\\res')
...
class ExternalTrajectoryData:
    ...
    def _read_csv(self):
        ...
        try:
            ...
            print(f"文件 {self.filename} 未找到，请检查文件路径。")
    ...
    def sys_trajectory(self):
        ...
        save_variable(bound_data, f'{self.direction}_trajectorydata', 'csptlib\\res')
...
if __name__ == "__main__":
    ...
    # ex = ExternalTrajectoryData('D:\\桌面文件\\第四章\\CSPT\\RateArterialData\\data.csv', 'inbound')
    # ex.sys_trajectory()
    # import time
