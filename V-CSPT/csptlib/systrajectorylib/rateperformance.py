...
def output_POG(single_purdue_data: dict, purdue_BOG_EOG_: dict) -> float:
    ...
    return round((sum(percent_on_green_set) / len(percent_on_green_set)) * 100, 2)
...
def output_stop_percent(inter_location: list, trajectories_data: dict, stop_speed_threshold: float) -> float:
    ...
    return round(sum(stop_percent) / len(stop_percent) * 100, 2)
...
def output_stop_delay(single_rate_data: dict, inter_num: int, filename: str, path: str):
    ...
    print('The average number of stops is:', round(stop_times / ((inter_num - 1) * num_veh), 3))
    ...
    return round(stop_times / ((inter_num - 1) * num_veh), 3), num_veh
...
class RateCoordinationPerformance:
    ...
    def __init__(self, rate_data: tuple[dict, dict] = None, inter_location: tuple[list, list] = None,
                 ...
                 dimensionless_constants: list = None, weight=None):
        """
        Initialize the data.
    ...
    def output_performance_grade(self) -> tuple:
        ...
        # 格式转换
        if set(self.ari_signal_plan_ring1.keys()) != {'yr1', 'green1', 'yr2', 'green2', 'red'}:
            self.ari_signal_plan_ring1 = self.convert_ring(self.ari_signal_plan_ring1)
            self.ari_signal_plan_ring2 = self.convert_ring(self.ari_signal_plan_ring2)
        # inbound
        # 计算交叉口类型
        ...
        aus_score_set, aip_score_set, \
            in_ave_speed, in_ave_speed_seed = self._calculate_single_performance(self.inbound_rate_data, IC_types,
        ...
        print('inbound average speed in the direction is:', round(in_ave_speed, 2))
        ...
        aus_score_set, aip_score_set, \
            out_ave_speed, out_ave_speed_seed = self._calculate_single_performance(self.outbound_rate_data, IC_types,
        ...
        print('outbound average speed in the direction is:', out_ave_speed)
        ...
        output_seed_speed(in_ave_speed_seed, 'inbound_seed_ave_speed', '.\\csptlib\\res\\')
        output_seed_speed(out_ave_speed_seed, 'outbound_seed_ave_speed', '.\\csptlib\\res\\')

        # 干线整体评级
        rpf = self.get_RPF()
        in_grade, in_description = self.determine_rate([in_grade, in_grade], rpf)
        out_grade, out_description = self.determine_rate([out_grade, out_grade], rpf)
        ari_grade, ari_description = self.determine_rate([in_grade, out_grade], rpf)
        print("The final rating for arterial coordination performance is:", ari_grade)
        print(ari_description)

        # 返回评估数据
        inbound_grade_set = [round(in_ave_speed, 2), round(inbound_ave_aip_score, 2), round(inbound_ave_aus_score, 2),
                             in_grade, in_description]
        outbound_grade_set = [round(out_ave_speed, 2), round(outbound_ave_aip_score, 2),
                              round(outbound_ave_aus_score, 2), out_grade, out_description]

        return ari_grade, ari_description, inbound_grade_set, outbound_grade_set
...
if __name__ == '__main__':
    ...
    # per.ari_signal_plan_ring1 = {'yr1': [3.0, 2.0, 2.0, 2.0], 'green1': [29.0, 43.0, 43.0, 52.0],
    #                              'yr2': [2.0, 2.0, 2.0, 2.0], 'green2': [39.0, 37.0, 38.0, 50.0],
    #                              'red': [75.0, 64.0, 63.0, 42.0]}
    # per.ari_signal_plan_ring2 = {'yr1': [3.0, 2.0, 2.0, 2.0], 'green1': [43.0, 39.0, 25.0, 39.0],
    #                              'yr2': [2.0, 2.0, 2.0, 2.0], 'green2': [25.0, 41.0, 56.0, 63.0],
    #                              'red': [75.0, 64.0, 63.0, 42.0]}
    #
